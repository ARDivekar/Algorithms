...
Examples (see URL https://community.topcoder.com/stat?c=problem_statement&pm=2998&rd=5857 for pictures)
...
void printGrid(bool** grid, int M, int N){
    cout<<endl;
    for(int i=0; i<M; i++){
        cout<<endl;
        for(int j=0; j<N; j++){
...
void setAreas(bool** grid, int M, int N, string areas[], int numAreas){
    int i,j,x,y;
    vector<string> rectCoordinates;
    for(x=0; x<numAreas; x++){
        rectCoordinates = split(areas[x], " ");
        // cout<<"\nSetting areas for: "<<areas[x]<<endl;
        for(i=atoi(rectCoordinates[0].c_str()); i <= atoi(rectCoordinates[2].c_str()); i++)
            for(j=atoi(rectCoordinates[1].c_str()); j <= atoi(rectCoordinates[3].c_str()); j++)
                grid[i][j] = 0;
    }
}
...
inline void pushAllDirections(bool** grid, int M, int N, bool** pushed, stack<Point> &S, Point &current, int &currentArea){
    int x=current.i;
    int y=current.j;
    int i,j;
    for(int k=1; k<=8; k++){
        switch(k){
            case 1: i=x-1;  j=y;    break;      // North
            case 2: i=x-1;  j=y+1;  break;      // North East
            case 3: i=x;    j=y+1;  break;      // East
            case 4: i=x+1;  j=y+1;    break;      // South East
            case 5: i=x+1;  j=y;    break;      // South
            case 6: i=x+1;  j=y-1;    break;      // South West
            case 7: i=x;    j=y-1;    break;      // West
            case 8: i=x-1;  j=y-1;    break;      // North West
        }
        if(inBounds(0, i, M-1) && inBounds(0, j, N-1) && grid[i][j]==true && pushed[i][j]==false){
            pushed[i][j] = true;
...
vector<int> getAreasByDFS(bool** grid, int M, int N){
    int i,j;
    bool** pushed = new bool*[M];
    for(i=0; i<M; i++){
        pushed[i] = new bool[N];
        for(j=0; j<N; j++)
            pushed[i][j] = false;
        ...
        for(j=0; j<N; j++){
            if(inBounds(0, i, M-1) && inBounds(0, j, N-1) && grid[i][j]==true && pushed[i][j]==false){
                ...
                while(!S.empty()){
                    current = S.top();
                    S.pop();
                    // cout << "\nDFS (" << current.i << "," << current.j <<")";
                    pushAllDirections(grid, M, N, pushed, S, current, currentArea);
                }
                if(currentArea>=1){
                    allAreas.push_back(currentArea);
                    // printGrid(pushed, M, N);
                }
            }
        }
    }
    sort(allAreas.begin(), allAreas.end());
    for(i=0; i<allAreas.size(); i++)
        cout<<allAreas[i]<<" ";
}


int main(){
    string areas[] = {"0 192 399 207", "0 392 399 407", "120 0 135 599", "260 0 275 599"};
    // Should return: { 22080,  22816,  22816,  23040,  23040,  23808,  23808,  23808,  23808 }
    // string areas[] = {"0 10 9 25", "5 0 7 39"};
    int numAreas = sizeof(areas)/sizeof(char*);
    int i,j;
    const int M = 400;
    const int N = 600;
    // const int M = 10;
    ...
    setAreas(grid, M, N, areas, numAreas);
    // printGrid(grid, M, N);
    getAreasByDFS(grid, M, N);
    return 0;
}
